...
import {
  ...
  intialSignupState,
  signupReducer,
} from "./SignupHelper/SignupHelper";
...
function Signup(props) {
  ...
  const handleSignup = () => {
    ...
    dispatchSignupState({ type: "PHONE_INPUT_BLUR" });
    dispatchSignupState({ type: "EMAIL_INPUT_BLUR" });
    const userFoundInLocalStorage = JSON.parse(
      localStorage.getItem(signupState.phoneNumber)
    );
  ...
  return (
    <div
      className={`${styles.sidenav} ${
        isSignupSidebarOpen ? styles["drawer_open"] : ""
      }`}
    >
      <div className={styles.cross} onClick={handleSignupSidebarClose}></div>
      <p className={styles.navSignup}>Sign up</p>
      <p className={styles.signup_acc}>
        <span>or </span>
        <span
          className={styles.loginYourAccount}
          onClick={handleLoginToYourAccount}
        >
          {" "}
      ...
      <div className={`${styles.signupinput} ${styles.input1}`}>
        {userAlreadyExists && (
          <p className={styles.userAlreadyExists}>
            User already exists. Please login to continue.
          </p>
        )}
        {signupState.isphoneNumberValid === false &&
          signupState.isPhoneNumberFocus === false && (
      ...
      <div className={`${styles.signupinput} ${styles.input2}`}>
        {signupState.isNameValid === false &&
          signupState.isNameFocus === false && <p>Name cannot be empty</p>}
        <input
          type="text"
          placeholder="Name"
      ...
      <div className={`${styles.signupinput} ${styles.input3}`}>
        {signupState.isEmailValid === false &&
          signupState.isEmailFocus === false && <p>Please enter valid email</p>}
        <input
          type="text"
          placeholder="Email"
      ...
      <p className={styles.referral}>Have a referral code?</p>
