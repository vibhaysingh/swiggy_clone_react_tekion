...
function Login(props) {
    ...
    const phoneNumberChangeHandler = (event) => {
        if (event.target.value === "" || checkNumberOnlyRegex.test(event.target.value)) {
            setIsUserFound(true);
            dispatchLoginState({ type: 'USER_PHONE_INPUT', phoneNumber: event.target.value });
    ...
    const handleLogin = () => {
        ...
        setTimeout(() => {
            ...
            setCurrentUser({ userName: userDetail.name, phoneNumber: userDetail.phoneNumber });
            localStorage.setItem("currentUser", JSON.stringify({ userName: userDetail.name, phoneNumber: userDetail.phoneNumber }));
            dispatchLoginState({ type: 'CLEAR_LOGIN' });
    ...
    return (
        ...
        <div className={`${styles.sidenav} ${isLoginSidebarOpen ? styles['drawer_open'] : ''}`}>
            <div className={styles.cross} onClick={handleLoginSidebarClose}></div>
            <p className={styles.navlogin}>Login</p>
            <p className={styles.create_acc}><span>or</span>
                <span className={styles.createAnAccount} onClick={handleCreateAnAccount}> create an account
                </span></p>
            <div className={styles.loginFood}><img src={sidebar_image} alt='' /></div>
            <div className={styles.loginInput}>

                {(loginState.isphoneNumberValid === false && loginState.isPhoneNumberFocus === false) && <p className={styles.invalidPhoneNumber}>Invalid Phone Number</p>}
                {!isUserFound && <p className={styles.userNotFound}>User not found! Please Signup.</p>}
                <input
                    type="text"
            ...
            <div className={styles.loginButton} onClick={handleLogin}><button>{loading ? 'Logging...' : 'LOGIN'}</button></div>
